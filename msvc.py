...
ap.add_argument(
    "--accept-license", action="store_true", help="Automatically accept license"
)
...
ap.add_argument(
    ...
    help=f"Visual Studio version to use for installation",
    choices=ALL_VERSIONS,
)
ap.add_argument(
    "--insiders", "--preview", action="store_true", help="Use insiders/preview versions"
)
ap.add_argument(
    "--target",
...
ap.add_argument(
    "--host", default=DEFAULT_HOST, help=f"Host architecture", choices=ALL_HOSTS
)
args = ap.parse_args()
...
for pid, p in packages.items():
    if (
        pid.startswith("Microsoft.VC.".lower())
        and pid.endswith(".Tools.HostX64.TargetX64.base".lower())
        and "premium" not in pid.lower()
    ):
...
if msvc_ver in msvc:
    ...
    msvc_ver = msvc_pid.removeprefix("microsoft.vc.").removesuffix(
...
for target in targets:
    msvc_packages += [
        ...
        # --- SPECTRE MITIGATED LIBS INJECTED HERE ---
        f"microsoft.vc.{msvc_ver}.crt.{target}.spectre.desktop.base",
        f"microsoft.vc.{msvc_ver}.crt.{target}.spectre.store.base",
        f"microsoft.vc.{msvc_ver}.spectre.{target}.base",
        f"microsoft.vc.{msvc_ver}.mfc.spectre.{target}.base",
        f"microsoft.vc.{msvc_ver}.atl.spectre.{target}.base",
        # --------------------------------------------
    ...
    if target in ["x86", "x64"]:
        msvc_packages += [
            ...
            # Add Spectre version of ASAN just in case
            f"microsoft.vc.{msvc_ver}.asan.{target}.spectre.base",
        ]
...
with tempfile.TemporaryDirectory(dir=DOWNLOADS) as d:
    ...
    for m in msi:
        subprocess.check_call(
            f'msiexec.exe /a "{m}" /quiet /qn TARGETDIR="{OUTPUT.resolve()}"'
        )
...
for target in targets:
    for f in [f"store", "uwp", "enclave", "onecore"]:
        shutil.rmtree(
            OUTPUT / "VC/Tools/MSVC" / msvcv / "lib" / target / f, ignore_errors=True
        )
    shutil.rmtree(
...
for arch in ["x86", "x64", "arm", "arm64"]:
    if arch not in targets:
        shutil.rmtree(
            OUTPUT / "Windows Kits/10/Lib" / sdkv / "ucrt" / arch, ignore_errors=True
        )
        shutil.rmtree(
            OUTPUT / "Windows Kits/10/Lib" / sdkv / "um" / arch, ignore_errors=True
        )
    if arch != host:
        shutil.rmtree(
...
for target in targets:
    (OUTPUT / "VC/Tools/MSVC" / msvcv / f"bin/Host{host}/{target}/vctip.exe").unlink(
        missing_ok=True
    )
